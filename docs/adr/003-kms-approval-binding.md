...
2. The complete canonical envelope binds approval ID, human principal, truck
...
6. Changed identity, incident, plan, revision, action, order, quantity, target,
   hash, expiration, or key version fails closed with zero activation mutations.
   Idempotent replay returns the original stable receipt.
